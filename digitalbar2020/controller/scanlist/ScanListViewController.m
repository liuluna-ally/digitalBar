...
- (void)viewDidLoad {
    ...
    @try {
        ...
        {
            //获取设备编号
            if([defaults objectForKey:@"NO"]){
                self.deviceNo = [[NSUserDefaults standardUserDefaults] objectForKey:@"NO"];
                //2.1.请求服务器，实现获取设备列表功能
                [self getDeviceList:self->_deviceNo];
            }
        };
...
-(void)loadData{
    NSLog(@"手动刷新了");
    [self getDeviceList:self.deviceNo];
    //停止刷新
...
-(void)getDeviceList:(NSString *) value{
    @try {
        ...
        //根据位置查询设备列表
        NSURL *url = nil;
        NSString *str = [@"no=" stringByAppendingString:value];
        NSString *str1 = [str stringByAppendingString:@"&"];
        NSString *str2 = [str1 stringByAppendingString:@"type=JSON"];
        NSString *str3 = [@"http://182.61.134.30/device/no?" stringByAppendingString:str2];
